"""Shared fixtures for gws-doc-action-extractor skill tests.

The mock ``gws`` binary intercepts all CLI calls without network access.
Scripts that call ``gws drive files export -o <path>`` receive document
content written by set_export_content(), enabling pattern-matching tests
without a real Google Docs connection.
...
GWS_STUB = textwrap.dedent("""\
    ...
    # Flatten args for pattern matching (collapse newlines within args)
...
# Metadata JSON returned for 'drive files get' calls
_DOC_METADATA = json.dumps({
    "id": "test-file-id-001",
    "name": "Test Meeting Notes",
    "mimeType": "application/vnd.google-apps.document",
    "modifiedTime": "2026-04-27T12:00:00Z",
    "webViewLink": "https://docs.google.com/document/d/test-file-id-001/edit",
})
...
def gws_env(tmp_path, monkeypatch):
    """Set up the mock gws binary for doc-action-extractor tests.

    Yields an Env object with helpers:
      .tmp_path           - pytest tmp_path
      .log_file           - Path where call logs are written
      .responses_file     - Path to JSON responses file
      .set_responses()    - write responses dict
      .set_doc_content()  - configure what 'drive files export' returns
      .read_log()         - get list of recorded calls
      .calls_contain()    - check if any call contains all given substrings
    """
    ...
    # Default: metadata GET succeeds, export returns empty content
    responses_file.write_text(json.dumps({
        "drive files get": [0, _DOC_METADATA],
        "drive files export": [0, ""],
    }))
    ...
    class Env:
        ...
        def set_doc_content(self, content: str) -> None:
            """Set the document text returned by 'drive files export'."""
            self.set_responses({
                "drive files get": [0, _DOC_METADATA],
                "drive files export": [0, content],
            })
